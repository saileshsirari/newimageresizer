...
//
    ...
    private double [][] [] transposeColorMatrix(double[][] [] colorData, int rows, int cols) {


        double [][] [] tranposedColorData = new double[cols][rows][3];//rows and columns are interchange
        for (int col = 0; col < cols; col++) {
            for (int row = 0; row < rows; row++) {
...
//                +" , "+tranposedColorData[0].length);
    ...
    private double calculateEnergy(int row, int col) {
        ...
        if (energy[col][row] == 0d || energy[col][row] != ENERGY_BOUNDARY) {
            ...
            double []  rgbXN = pictureData[col + 1][row];
            double []  rgbXP = pictureData[col - 1][row];
            double []  rgbYP = pictureData[col][row - 1];
            double []  rgbYN = pictureData[col][row + 1];


            int endInd =2;
            double deltax = Math.pow(rgbXN[endInd] - rgbXP[endInd], 2) +
                    Math.pow(rgbXN[endInd-1] - rgbXP[endInd-1], 2) +
                    Math.pow(rgbXN[endInd-2] - rgbXP[endInd-2], 2) ;

            double deltay =Math.pow(rgbYN[endInd] - rgbYP[endInd], 2) +
                    Math.pow(rgbYN[endInd-1] - rgbYP[endInd-1], 2) +
                    Math.pow(rgbYN[endInd-2] - rgbYP[endInd-2], 2) ;
...
//now we have vertices in toplogical order
            Stack<SeamCarverOpenCv.Point> topologicalOrder = this.reversePost();
...
//                }
            while (topologicalOrder.isEmpty() == false) {
                SeamCarverOpenCv.Point p = topologicalOrder.pop();
